...
public final class ReferenceTransferCapableConvergenceLayerAdapter: @unchecked Sendable, ConvergenceLayerAdapter {
    public let bearerID: BearerID

    private let capabilityTimeScope: TimeScope
    private let nowUnixMs: @Sendable () -> UInt64
    private let eventStreams = TransferSessionEventStreams()
    ...
    public func currentCapabilitySnapshot(nowUnixMs _: UInt64) -> BearerCapabilitySnapshot {
        BearerCapabilitySnapshot(
            bearerID: bearerID,
            supportedLanes: [.discovery, .control, .data],
...
private final class TransferSessionEventStreams: @unchecked Sendable {
