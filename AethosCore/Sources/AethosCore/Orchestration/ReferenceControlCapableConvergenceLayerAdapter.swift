...
public final class ReferenceControlCapableConvergenceLayerAdapter: @unchecked Sendable, ConvergenceLayerAdapter {
    public let bearerID: BearerID

    private let capabilityTimeScope: TimeScope
    private let nowUnixMs: @Sendable () -> UInt64
    private let eventStreams = ControlSessionEventStreams()
    ...
    public func currentCapabilitySnapshot(nowUnixMs _: UInt64) -> BearerCapabilitySnapshot {
        BearerCapabilitySnapshot(
            bearerID: bearerID,
            supportedLanes: [.discovery, .control],
...
private final class ControlSessionEventStreams: @unchecked Sendable {
