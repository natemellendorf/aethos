...
/// File-system-based Link implementation for local peer-to-peer transport.
///
/// ## Directory contract
///
/// | Directory | Owner     | Contents |
/// |-----------|-----------|----------|
/// | **outbox/**   | `send()`  | Outbound frames waiting for physical transport. Files use the `out-` prefix. An external courier (script, sync agent, etc.) moves these to the remote peer's inbox. |
/// | **inbox/**    | external  | Inbound frames deposited by an external courier. `receive()` reads the oldest file, decodes the frame, then moves the file to archive. |
/// | **archive/**  | `receive()` | Processed inbox files. Successfully decoded files are renamed from `out-*` to `in-*` to distinguish them from locally-generated outbound files. Malformed files receive a `.bad` suffix. |
///
/// ## File prefix semantics
///
/// - `out-` — generated by `send()`. Only appears in outbox (or in archive if an external script copies them back).
/// - `in-`  — renamed by `archiveGood()` when processing an inbox file that originally had an `out-` prefix.
///   This prevents external couriers that glob `out-*.bin` from re-forwarding already-processed frames.
///
/// ## Important
///
/// Archive is a **terminal** directory — files there have already been processed and should NOT
/// be forwarded again. If a courier needs to forward response frames (acks, receipts), those
/// will appear in the peer's **outbox**, not its archive.
