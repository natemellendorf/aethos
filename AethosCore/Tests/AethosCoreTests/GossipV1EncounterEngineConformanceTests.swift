...
func gossipV1_summaryDatagram_isDeterministic_forFixedEligibleSet() throws {
    ...
    let frameA = try engine.buildSummary(clock: clock, store: store)
    ...
    guard case .summary(let summaryFrame) = frameA else {
        return #expect(Bool(false), "expected summary frame")
    }
    #expect(summaryFrame.itemCount == 3)
    let previewExpected = [idA, idB, idC].sorted(by: {
