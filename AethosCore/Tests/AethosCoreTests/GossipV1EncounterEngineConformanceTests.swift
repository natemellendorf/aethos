...
@Test
func gossipV1_engine_enforcesPeerHelloMaxWant_onInboundRequest() throws {
    ...
    let request = try GossipV1RequestFrame(want: ids)
    // Inbound REQUEST must be capped by our own advertised localHello.maxWant (and MAX_WANT_ITEMS),
    // not by the peer's max_want (which bounds what we send).
