...
| CRP-PAYLOAD-BASE64URL | `payload_b64` canonical encoding | `payload_b64` MUST be an unpadded base64url string that encodes canonical `EnvelopeV1` bytes. | DIVERGES | OK | both | IN_PROGRESS | bead/crp-payload-base64url | Spec: `docs/spec/CLIENT_RELAY_PROTOCOL_V1.md#1-transport-and-encoding`; Audit: `relay#confirmed-divergences` / `ios#confirmed-matches`; Notes: iOS emits canonical unpadded base64url outbound; iOS inbound decode remains migration-tolerant (base64/base64url, padded/unpadded); relay stores and forwards `payload_b64` as an opaque string (no decode/validation/canonicalization), so relay outbound may be non-canonical when non-canonical input was stored. |
...
| DELIV-PER-DEVICE-ACK-BINDING | Per-device delivery keying | Delivery/ack must bind to `(wayfarer_id, device_id, msg_id)` and not cross-suppress devices. | DIVERGES | VERIFY | both | TODO | - | Spec: `docs/spec/CLIENT_RELAY_PROTOCOL_V1.md#63-per-device-tracking-and-ack-binding`; Audit: `relay#confirmed-divergences` / `ios:-` |
