...
| CRP-PAYLOAD-BASE64URL | `payload_b64` canonical encoding | `payload_b64` must be unpadded base64url canonical `EnvelopeV1` bytes. | DIVERGES | OK | both | IN_PROGRESS | bead/crp-payload-base64url | Spec: `docs/spec/CLIENT_RELAY_PROTOCOL_V1.md#1-transport-and-encoding`; Audit: `relay#confirmed-divergences` / `ios#confirmed-matches`; Notes: iOS emits canonical unpadded base64url outbound; iOS inbound decode remains migration-tolerant (base64/base64url, padded/unpadded); relay treats payload as opaque string (no validation/canonicalization), and outbound mirrors stored payload. |
