...
| Feature ID | Feature | Spec Expectation | Relay Status | iOS Status | Owner | Migration Status | Alignment Bead | Spec Reference |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| ERR-ERROR-FRAME-SCHEMA | Structured error payload schema | `error` must include `code` and `message` fields. | OK | OK | both | COMPLETE | - | Spec: `docs/spec/CLIENT_RELAY_PROTOCOL_V1.md#32-error`; Audit: `relay#confirmed-matches` / `ios#confirmed-matches`; Notes: `error.msg_id` is a legacy compatibility field (not required by v1 spec); relay may temporarily populate `error.msg_id` as an alias of `error.message` for older clients, and iOS consults `error.msg_id` only when structured `error.code`/`error.message` are absent. |
