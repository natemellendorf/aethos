...
| Feature ID | Feature | Spec Expectation | Relay Status | iOS Status | Owner | Migration Status | Alignment Bead | Spec Reference |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| CRP-HELLO-DEVICE-ID | `hello.device_id` required | `hello` must include `wayfarer_id` + `device_id` in v1. | DIVERGES | OK | both | IN_PROGRESS | aethos-b3n | Relay accepts optional `device_id`, derives delivery identity from `(wayfarer_id, device_id)` when present, and falls back to wayfarer-only identity when missing for backward compatibility; queue indexing is still wayfarer-scoped with per-device filtering. iOS now persists a stable `device_id` and sends it in `hello`. Spec: `docs/spec/CLIENT_RELAY_PROTOCOL_V1.md#31-hello`; Audit: `relay#confirmed-divergences` / `ios#confirmed-matches` |
